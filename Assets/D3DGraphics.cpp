...
void D3DGraphics::DravCircle(int cx, int cy, int rad, int r, int g, int b)
{
	float radSqr = rad*rad;
	float xwidth = sqrt(2.0) / 2 * rad;
	
	for (int x = -xwidth; x <= xwidth; x++)
	{
		int y = sqrt(radSqr - x*x);
		PutPixel(x + cx, y + cy, r, g, b);
		PutPixel(x + cx, cy - y, r, g, b);
	}
	for (int y = -xwidth; y <= xwidth; y++)
	{
		int x = sqrt(radSqr - y*y);
			PutPixel(x + cx, y + cy, r, g, b);
